<footer class="admin-footer">
  &copy; <?= date('Y') ?> Sagay City Farm School — E-Module LMS. All rights reserved.
</footer>

<!-- Skeleton Page Loader -->
<div id="page-skeleton" style="display:none;position:fixed;inset:0;z-index:9999;background:#F8FAF7;overflow:hidden;">
  <style nonce="<?= csp_nonce() ?>">
    ...
    #sk-sidebar{width:220px;min-height:100vh;background:#0f2410;flex-shrink:0;}
    ...
    .sk-bottom-grid{display:grid;grid-template-columns:2fr 1fr;gap:12px;}
    @media(max-width:767px){
      #sk-sidebar,#sk-topbar{display:none!important;}
      #sk-body{padding:16px 12px;}
      .sk-cards{grid-template-columns:repeat(2,1fr);}
      .sk-bottom-grid{grid-template-columns:1fr;}
    }
    @media(max-width:479px){
      .sk-cards{gap:8px;}
      #sk-body{padding:12px 10px;}
    }
  </style>
  <div style="display:flex;min-height:100vh;">
    <div id="sk-sidebar"></div>
    <div style="flex:1;display:flex;flex-direction:column;">
      ...
      <div id="sk-body">
        ...
        <div class="sk-bottom-grid">
          <div class="sk-card">
            <div class="skel" style="width:40%;height:13px;margin-bottom:14px;"></div>
            <?php for($i=0;$i<4;$i++): ?>
            <div style="display:flex;gap:10px;margin-bottom:12px;align-items:center;">
              <div class="skel" style="width:28px;height:28px;border-radius:50%;flex-shrink:0;"></div>
              <div style="flex:1;"><div class="skel" style="width:65%;height:11px;margin-bottom:5px;"></div><div class="skel" style="width:45%;height:9px;"></div></div>
            </div>
            <?php endfor; ?>
          </div>
          <div class="sk-card">
            <div class="skel" style="width:60%;height:13px;margin-bottom:14px;"></div>
            <?php for($i=0;$i<3;$i++): ?>
            <div class="skel" style="width:100%;height:34px;margin-bottom:10px;border-radius:10px;"></div>
            <?php endfor; ?>
          </div>
        </div>
      </div>
...
(function(){
  ...
  function showSkeleton(){
    if(window.innerWidth <= 767){
      sk.style.top='52px'; sk.style.bottom='58px';
    } else {
      sk.style.top='0'; sk.style.bottom='0';
    }
    sk.style.left='0'; sk.style.right='0';
  ...
  window.addEventListener('pageshow', function(e){
    if(e.persisted){ hideSkeleton(); sp.style.display='none'; }
  });
